...
{
    #ifdef ENET_DEBUG_PROTOCOL
    printf("enet_protocol_handle_connect\n");
    #endif
    ...
    #ifdef ENET_DEBUG_PROTOCOL
    printf("Connect: Channel Count: %X\n", channelCount);
    #endif
    ...
    for (currentPeer = host->peers; currentPeer < & host->peers [host->peerCount]; ++currentPeer)
        if (currentPeer->state != ENET_PEER_STATE_DISCONNECTED && currentPeer->address.host == host->receivedAddress.host && currentPeer->address.port == host->receivedAddress.port && currentPeer->sessionID == command->connect.sessionID)
        {
            #ifdef ENET_DEBUG_PROTOCOL
            printf("Connect: failed because there is an identical connection already.\n");
            #endif
    ...
    {
        #ifdef ENET_DEBUG_PROTOCOL
        printf("Connect: failed because of Invalid peer (memory)!\n");
        #endif
    ...
    {
        #ifdef ENET_DEBUG_PROTOCOL
        printf("Connect: failed to allocate memory for channels\n");
        #endif
    ...
    #ifdef ENET_DEBUG_PROTOCOL
    printf("Got to veriy_connect answer\n");
    #endif
...
{
    #ifdef ENET_DEBUG_PROTOCOL
    printf("enet_protocol_handle_send_reliable\n");
    #endif
    ...
    * currentData += dataLength;
    ...
    packet = enet_packet_create ((const enet_uint8 *) command + sizeof (ENetProtocolSendReliable), dataLength, ENET_PACKET_FLAG_RELIABLE);

    if (packet == NULL || enet_peer_queue_incoming_command (peer, command, packet, 0) == NULL)
        return -1;

    #ifdef ENET_DEBUG_PROTOCOL
    printf("Send reliable done successfully\n");
    #endif
...
enet_protocol_handle_incoming_commands (ENetHost * host, ENetEvent * event)
{
    ...
    #ifdef ENET_DEBUG_PROTOCOL
    for (int i = 0; i < host->receivedDataLength; i++)
    ...
    if ((peerID & ENET_PROTOCOL_HEADER_FLAG_MASK) == ENET_PROTOCOL_HEADER_FLAG_MASK)
        peerID &= ~ENET_PROTOCOL_HEADER_FLAG_MASK;

    flags = header->flag & ENET_PROTOCOL_HEADER_FLAG_MASK;

    #ifdef ENET_DEBUG_PROTOCOL
    printf("Host peercount: %d\n", host->peerCount);
    printf("Received PeerID %X and Flag %X\n", peerID, flags);
    ...
    if((header->flag & 0x7F) == ENET_PROTOCOL_MAXIMUM_PEER_ID)
    {
        ...
        #ifdef ENET_DEBUG_PROTOCOL
        printf("Found appropriate flag, peer = null done\n");
        #endif
    }
    else if (peerID >= host->peerCount)
    {
        #ifdef ENET_DEBUG_PROTOCOL
        printf("PeerID is bigger than the hosts peerCount, leaving :(\n");
        #endif
    ...
    else
    {
        ...
        if (peer->state == ENET_PEER_STATE_DISCONNECTED || peer->state == ENET_PEER_STATE_ZOMBIE || ((host->receivedAddress.host != peer->address.host || host->receivedAddress.port != peer->address.port) && peer->address.host != ENET_HOST_BROADCAST))
            return 0;
    ...
    {
        #ifdef ENET_DEBUG_PROTOCOL
        printf("Started processing information after the intial header\n");
        #endif
       ...
       commandNumber = command->header.command & ENET_PROTOCOL_COMMAND_MASK;
        ...
        #ifdef ENET_DEBUG_PROTOCOL
        printf("Command number: %X ; Header.command: %X\n", commandNumber, command->header.command);
        #endif
       ...
       {
            #ifdef ENET_DEBUG_PROTOCOL
            printf("Exiting at command size check, command size: %X\n", commandSize);
            #endif
...
enet_protocol_send_outgoing_commands(ENetHost * host, ENetEvent * event, int checkForTimeouts)
{
    ...
    while (host->continueSending)
    {
        ...
        for (; currentPeer < &host->peers[host->peerCount]; ++currentPeer)
        {
            if (currentPeer->state == ENET_PEER_STATE_DISCONNECTED ||
            ...
            header.serverDetailsCRCHash = 0x09294C9B;
            header.hmacKey = 0;
